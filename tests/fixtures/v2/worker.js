    ...
    for (const item of newItems) {
      ...
      try {
        ...
        const detail = {
          ...
          categories,
          explanation,
          originalUrl: item.originalUrl || makeTdnetListUrl(result.date),
...
function buildExplanation(title, categories) {
  if (isThirdPartyInvestmentByCompany(title)) {
    return {
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    for (const row of rows) {
      ...
      pageItems.push({
        ...
        company,
        title,
        originalUrl: originalUrl || makeTdnetListUrl(date)
      });
    }
...
async function sendPush(env, item, categories, detailId, date) {
  const detailUrl =
    `${APP_BASE_URL}detail.html?id=${encodeURIComponent(detailId)}` +
